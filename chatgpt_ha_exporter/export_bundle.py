...
def load_options() -> dict[str, Any]:
    ...
    if OPTIONS_PATH.exists():
        try:
            ...
            print(f"WARN: Could not read options.json: {exc}")
    return options
...
def export_raw_text(src: Path, dst: Path, stats: ExportStats) -> None:
    try:
        ...
        stats.warnings.append(f"Could not export text file: {src} ({exc})")
def export_yaml(src: Path, raw_dst: Path, normalized_dst: Path, stats: ExportStats) -> None:
    ...
    try:
        ...
        stats.warnings.append(f"YAML normalization failed for {src}: {exc}")
def export_json(src: Path, raw_dst: Path | None, normalized_dst: Path, stats: ExportStats, *, storage_name: str | None = None) -> None:
    if raw_dst is not None:
        try:
            ...
            stats.warnings.append(f"Could not export raw JSON file {src}: {exc}")
    try:
        ...
        stats.warnings.append(f"JSON normalization failed for {src}: {exc}")
def copy_tree_sanitized(src_root: Path, dst_root: Path, stats: ExportStats) -> None:
    if not src_root.exists():
        stats.excluded_items.append(f"Missing: {src_root}")
        return
...
def try_api_json(url: str, stats: ExportStats) -> tuple[bool, Any]:
    try:
        ...
        stats.warnings.append(f"API call failed {url}: {exc}")
        return False, None
def try_api_text(url: str, stats: ExportStats, accept: str = "text/plain") -> tuple[bool, str | None]:
    try:
        ...
        stats.warnings.append(f"API text call failed {url}: {exc}")
        return False, None
...
def detect_theme_include(configuration_text: str) -> dict[str, Any]:
    ...
    if not path.exists():
        ...
        result["note"] = f"configuration.yaml declares frontend/themes via {result['directive']} {result['include_target']}, but directory {path} is missing."
        return result
    if not path.is_dir():
        result["status"] = "unreadable"
        result["note"] = f"Theme path {path} is not a readable directory."
        return result
    try:
        files = sorted([p for p in path.iterdir() if p.is_file()])
    except Exception:
        result["status"] = "unreadable"
        result["note"] = f"Directory {path} was not readable."
        return result
    if not files:
        result["status"] = "empty"
        result["note"] = f"Directory {path} is empty."
        return result
    ...
    if result["yaml_files"]:
        ...
        result["note"] = f"Only non-YAML files were found in {path}."
    return result
...
def export_config_files(export_dir: Path, options: dict[str, Any], stats: ExportStats) -> dict[str, Any]:
    ...
    details["directories"] = list(dict.fromkeys(details["directories"]))
    details["yaml_files"] = list(dict.fromkeys(details["yaml_files"]))
...
def export_storage(export_dir: Path, options: dict[str, Any], stats: ExportStats) -> dict[str, Any]:
    ...
    if not storage_root.exists():
        stats.excluded_items.append(".storage missing")
...
def export_recorder_summary(export_dir: Path, options: dict[str, Any], stats: ExportStats) -> dict[str, Any]:
    ...
    if not db_path.exists():
        stats.excluded_items.append("Recorder DB missing")
        return summary
    try:
        ...
        if "states" in tables and "states_meta" in tables:
            try:
                ...
                stats.warnings.append(f"Recorder entity summary failed: {exc}")
        ...
        if options.get("include_recorder_deep_export", True):
            ...
            if table_exists(cursor, "events"):
                ...
                try:
                    ...
                    stats.warnings.append(f"Recorder event type counts failed: {exc}")
        ...
        stats.warnings.append(f"Recorder export failed: {exc}")
        return summary
def build_relationship_integrity_report(export_dir: Path, stats: ExportStats) -> dict[str, Any]:
    ...
    try:
        ...
        stats.warnings.append(f"Relationship integrity report failed: {exc}")
        return report
...
def determine_output_targets(export_name: str, options: dict[str, Any], stats: ExportStats) -> OutputTargets:
    ...
    if not ok:
        raise RuntimeError(f"/share is not writable: {msg}")
...
def mirror_output(source_dir: Path, source_archive: Path, target: OutputTargetResult, stats: ExportStats) -> None:
    if target.export_dir is None or target.archive_path is None:
        target.ok = False
        target.message = "Target paths missing"
        return
    try:
        ...
        stats.warnings.append(f"Mirroring to {target.label} failed: {exc}")
...
def build_uncertainty_register(export_dir: Path, options: dict[str, Any], stats: ExportStats, config_info: dict[str, Any], storage_info: dict[str, Any], logs_info: dict[str, Any], recorder_info: dict[str, Any], backup_info: dict[str, Any], operator_intent_info: dict[str, Any], integration_profiles: dict[str, Any], addon_options_info: dict[str, Any]) -> dict[str, Any]:
    ...
    if not logs_info.get("files") and not logs_info.get("tails") and not runtime_logs_present:
        add_item(
            ...
            "No home-assistant.log was found in the export even though runtime and failure analysis would benefit strongly from it.",
            [{"source": "logs_info", "fact": "neither a full log file nor a tail export was produced"}],
    ...
    for name, item_id in critical_storage.items():
        if name in available_only:
            add_item(item_id, "export_scope_gap", "medium", f"Storage file not exported: {name}", f"{name} was visible in the system but was not included in the export.", [{"source": "inventory/storage_inventory.json", "fact": f"{name} in available_only"}], [f"include {name} in the export scope"])
        elif name not in included and name not in available_only:
            add_item(item_id, "hard_export_gap", "low", f"Storage file unavailable: {name}", f"{name} was not present in the export; either the file is truly missing or that area is not active in this installation.", [{"source": "inventory/storage_inventory.json", "fact": f"{name} not in included/available_only"}], [f"verify whether {name} exists in this installation and is relevant"])
    if not recorder_info.get("deep_exported"):
        add_item("UNC-RECORDER-001", "export_scope_gap", "high", "Recorder deep export missing", "There are no deeper recorder samples, so performance and execution analysis remain coarse.", [{"source": "runtime/recorder_deep_export_summary.json", "fact": "deep_exported=false or file missing"}], ["enable include_recorder_deep_export", "optionally export a recorder DB copy only when needed"])
    if not operator_intent_info.get("imported"):
        add_item("UNC-INTENT-001", "principled_uncertainty", "medium", "Operator intent missing", "Operator intent was not imported, so target architecture and no-go areas remain fundamentally uncertain.", [{"source": "metadata/operator_intent_import_manifest.json", "fact": "no operator_intent file imported"}], ["add operator_intent.md or operator_intent.json"])
    ...
    if esphome_profile and not (export_dir / "source_sanitized" / "config" / "esphome").exists() and (esphome_profile.get("config_entry_count") or esphome_profile.get("entity_count")):
        add_item("UNC-ESPHOME-001", "export_scope_gap", "high", "ESPHome source files missing", "ESPHome is clearly present in the system, but the source YAML files are missing from the export.", [{"source": "inventory/integration_profiles/esphome_profile.json", "fact": "entities/config_entries present, source_directories_present.esphome=false"}], ["export /config/esphome/*.yaml"])
    if addon_options_info.get("enabled") and not addon_options_info.get("exported"):
        add_item("UNC-ADDON-001", "export_scope_gap", "medium", "Addon options profiles missing", "Targeted add-on option profiles could not be produced even though the feature is enabled.", [{"source": "inventory/addon_options_profiles.json", "fact": "not generated or empty"}], ["check Supervisor access to /addons and /addons/<slug>/info"])
...
def generate_export_summary(export_dir: Path, options: dict[str, Any], stats: ExportStats, config_info: dict[str, Any], storage_info: dict[str, Any], api_info: dict[str, Any], runtime_info: dict[str, Any], recorder_info: dict[str, Any], backup_info: dict[str, Any], logs_info: dict[str, Any], relationship_info: dict[str, Any], security_info: dict[str, Any], operator_intent_info: dict[str, Any], addon_options_info: dict[str, Any], integration_profiles: dict[str, Any], uncertainty_info: dict[str, Any], targets: OutputTargets) -> None:
    ...
    included_hint = [
        ...
        "Normalized JSON/NDJSON files",
        "Entity/device/area registries with stable pseudonymization",
        "Config entries (sanitized)",
        "Add-on and system inventory",
        "Helper source definitions" if options.get("include_helper_source_definitions", True) else "No helper source definitions",
        "Template source definitions" if options.get("include_template_source_definitions", True) else "No template source definitions",
        "Current state snapshot" if options.get("include_current_state_snapshot", True) else "No state snapshot",
        "Service catalog" if options.get("include_service_catalog", True) else "No service catalog",
        "Recorder summary" if options.get("include_recorder_summary", True) else "No recorder summary",
        "Recorder deep export" if options.get("include_recorder_deep_export", True) else "No recorder deep export",
        "Multiple state snapshots" if options.get("include_multi_state_snapshots", True) else "Only one snapshot",
        "Deep backup/restore context" if options.get("include_backup_deep_context", True) else "No deep backup context",
        "HA/core/supervisor/add-on logs" if options.get("include_logs", True) else "No logs",
        "ESPHome/python_scripts/pyscript/AppDaemon directories when present",
        "Security/exposure report" if options.get("include_security_exposure_report", True) else "No security/exposure report",
        "Operator intent import" if options.get("include_operator_intent_import", True) else "No operator intent import",
        "Targeted add-on options export" if options.get("include_addon_options_export", True) else "No add-on options export",
        "Critical integration profiles" if options.get("include_integration_profiles", True) else "No integration profiles",
        "Machine-readable uncertainty register" if options.get("include_uncertainty_register", True) else "No uncertainty register",
    ]
    excluded_by_design = [
        "Raw secret values",
        "Raw tokens and passwords",
        "SSL keys and certificates",
        "Full recorder database (unless include_recorder_db_copy=true)",
        "Media files",
        "Non-referenceable random IDs without structural meaning",
    ]
    lines = [
        ...
        "",
        "## Purpose",
        "This export is intended for another ChatGPT instance so it can analyze, improve, extend, and refactor the setup with as few wrong assumptions as possible.",
        "",
        "## Saved output locations",
        f"- User-facing archive: {targets.primary.archive_path}",
    ]
    for mirror in targets.mirrors:
        lines.append(f"- Mirror archive: {mirror.archive_path} ({'ok' if mirror.ok else 'error'})")
    ...
    lines.extend([
        "",
        "## Important starting files for ChatGPT",
        ...
        *(["- metadata/operator_intent_import_manifest.json"] if operator_intent_import_manifest_exists else []),
        "",
        "## Observations",
        f"- Exported YAML files: {len(config_info.get('yaml_files', []))}",
        f"- Exported directories: {', '.join(config_info.get('directories', [])) or 'none'}",
        f"- Exported .storage files: {len(storage_info.get('included', []))}",
        f"- Exported log files: {', '.join(logs_info.get('all_exported_logs', [])) or 'none'}",
        f"- API calls: {len(stats.api_calls)}",
        f"- Recorder exported: {'yes' if recorder_info.get('exported') else 'no'}",
        f"- Recorder deep export: {'yes' if recorder_info.get('deep_exported') else 'no'}",
        f"- Multi-snapshot count: {runtime_info.get('captured_snapshot_count', 0)}",
        f"- Backup context exported: {'yes' if backup_info.get('exported') else 'no'}",
        f"- Theme status: {config_info.get('theme_include', {}).get('status', 'unknown')}",
        f"- Relationship-Integrity-Report: {'yes' if relationship_info.get('generated') else 'no'}",
...
## Target architecture
- What architecture do you want in the medium term?
- Which integrations/add-ons are strategically desired?
## Critical automations
- Which automations must never be changed unintentionally?
- Which automations are security-critical?
## Naming rules
- Preferred language: German / English / hybrid
- Rules for entity_id values, helpers, dashboards, areas, and labels
## Backup strategy
- Primary backup solution
- Retention
- External backup targets
## Intentionally retained legacy behavior
- Which legacy elements should intentionally remain?
## Never change automatically
- List of sensitive devices/functions/areas
"""
...
def build_security_exposure_report(export_dir: Path, options: dict[str, Any], stats: ExportStats) -> dict[str, Any]:
    ...
    if http_summary["use_x_forwarded_for"] or http_summary["trusted_proxies_count"]:
        add_finding("SEC-HTTP-001", "Reverse proxy context detected", "info", {"use_x_forwarded_for": http_summary["use_x_forwarded_for"], "trusted_proxies_count": http_summary["trusted_proxies_count"]}, "HTTP/proxy context is relevant and should be considered during security and exposure changes.")
    if http_summary["ssl_certificate_configured"] or http_summary["ssl_key_configured"]:
        add_finding("SEC-HTTP-SSL-001", "Direct SSL configuration detected in YAML", "info", {"ssl_certificate_configured": http_summary["ssl_certificate_configured"], "ssl_key_configured": http_summary["ssl_key_configured"]}, "The setup appears to carry or reference direct TLS context in Home Assistant.")
    if (provider_count or 0) > 0 or (users_count or 0) > 0:
        add_finding("SEC-AUTH-001", "Authentication context exported", "info", {"provider_count": provider_count, "users_count": users_count, "owner_count": owners, "active_users": active_users}, "User and provider structure is at least partially visible.")
    elif not auth_present or not provider_present:
        add_finding("SEC-AUTH-UNKNOWN", "Authentication context only partially exported", "low", {"auth_present": auth_present, "provider_present": provider_present}, "At least one relevant auth source is missing, so user/provider counts are only partially reliable.")
    if exposed_count > 0 or cloud_cfg_present:
        add_finding("SEC-EXPOSE-001", "Expose/voice-assistant context detected", "info", {"exposed_entities_count": exposed_count, "cloud_or_voice_config_present": cloud_cfg_present}, "Exposed entities and voice-assistant context are part of the optimization and security evaluation.")
    if app_cred_count > 0:
        add_finding("SEC-APPCREDS-001", "Application credentials present", "info", {"application_credentials_count": app_cred_count}, "Integrations with external services are likely present; changes should consider credential context.")
    if mobile_app_count > 0:
        add_finding("SEC-MOBILE-001", "Mobile app context present", "info", {"mobile_app_count": mobile_app_count}, "Mobile devices, triggers, and notifications may be part of critical behavior.")
    if assist_pipeline_count > 0:
        add_finding("SEC-ASSIST-001", "Assist pipelines present", "info", {"assist_pipeline_count": assist_pipeline_count}, "Voice/Assist pipelines are configured and should be considered during restructuring.")
    if (adapter_count or 0) > 0 or external_like:
        add_finding("SEC-NET-001", "Network/reachability context present", "info", {"adapter_count": adapter_count, "url_like_values_count": len(external_like)}, "Network and reachability data are partially visible; change HTTP/cloud/proxy settings carefully.")
    if frontend_panel_count > 0 or frontend_user_data_files or frontend_system:
        add_finding("SEC-UI-001", "Frontend/panel context present", "info", {"frontend_panel_count": frontend_panel_count, "frontend_user_data_files": frontend_user_data_files[:20]}, "Additional panels or user-specific frontend context are visible in the export.")
    info = {
        "generated": True,
        "sources": {
